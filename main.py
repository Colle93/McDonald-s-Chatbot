...
def diag():
    return JSONResponse(content={
        ...
        "has_chat_namespace": hasattr(OpenAI(), "chat")
    }, status_code=200)
...
def start_conversation():
    try:
        ...
        try:
            ...
            print("[THREAD] created (chat.*):", thread.id)
            return JSONResponse(content={"thread_id": thread.id}, status_code=200)
        except Exception as e_chat:
            print("[WARN] chat.threads.create failed → fallback beta:", e_chat)

        # fallback “storico”
        thread = client.beta.threads.create()
        print("[THREAD] created (beta.*):", thread.id)
        return JSONResponse(content={"thread_id": thread.id}, status_code=200)

    except Exception as e:
        # restituisci l’errore in JSON per capire subito dal browser
